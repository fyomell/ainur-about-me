...
export const getServerSideProps: GetServerSideProps<Props> = async (ctx) => {
  ...
  const chapter = getChapter("astronomy", chapterKey);
  if (!chapter) {
    return { redirect: { destination: "/learn/astronomy", permanent: false } };
  }
  ...
  if (p !== page) {
    return {
      redirect: { destination: `/learn/astronomy/${chapterKey}/${p}`, permanent: false },
    };
  }
...
export default function AstroReader({ chapterKey, page }: Props) {
  const chapter = getChapter("astronomy", chapterKey)!;
  ...
  const imgSrc = data.image || makeLessonImage(`astro-${chapterKey}`, page, data.title);
        ...
        <header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/55 backdrop-blur">
          <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-3">
            <div className="min-w-0">
              <div className="text-xs font-extrabold tracking-widest text-white/60">ASTRONOMI</div>
            ...
            <div className="flex gap-2">
              <Link href="/learn/astronomy" className="rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-black hover:bg-white/10">
                MENU
              </Link>
        ...
        <section className="mx-auto max-w-5xl px-4 pb-14 pt-8">
          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-soft backdrop-blur">
            ...
            <div className="mt-6 flex items-center justify-between gap-3">
              <Link href={`/learn/astronomy/${chapterKey}/${prev}`} className="rounded-2xl border border-white/10 bg-white/5 px-5 py-2.5 text-sm font-black hover:bg-white/10">
                PREV
              </Link>
              <Link href={`/learn/astronomy/${chapterKey}/${next}`} className="rounded-2xl bg-sky-500/90 px-5 py-2.5 text-sm font-black text-slate-950 hover:bg-sky-400">
                NEXT
              </Link>
            </div>
          </div>

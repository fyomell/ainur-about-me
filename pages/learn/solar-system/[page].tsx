...
export default function SolarSystemLesson({ lesson }: { lesson: LearnPage }) {
  const page = lesson.page;
  const total = SOLAR_SYSTEM_TOTAL;
  const prev = page > 1 ? `/learn/solar-system/${page - 1}` : null;
  const next = page < total ? `/learn/solar-system/${page + 1}` : null;
    ...
    <main className="min-h-screen">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0 leading-tight">
              <div className="truncate text-sm font-extrabold tracking-wide">
                BAB 1 • TATA SURYA • HALAMAN {page}/{total}
              </div>
              <div className="text-xs text-white/60">•DETAIL •RINCI •ASTRONOMI</div>
            </div>
          </div>
      ...
      <section className="mx-auto max-w-5xl px-4 pb-14 pt-8">
        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-soft backdrop-blur">
          ...
          <img src={lesson.image} alt="Solar System" className="mt-5 w-full rounded-2xl border border-white/10" />
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="text-xs text-white/60">•LOMPAT HALAMAN: /1 /2 /3 ... /20</div>
          </div>
        </div>
      </section>
    </main>
...
export const getStaticPaths: GetStaticPaths = async () => {
  const paths = Array.from({ length: SOLAR_SYSTEM_TOTAL }, (_, i) => ({
...
export const getStaticProps: GetStaticProps = async (ctx) => {
  ...
  const lesson = getSolarSystemPage(p);
